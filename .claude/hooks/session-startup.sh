#!/bin/bash
# LSDO Build Template — Session Startup Script
# Lives at: .claude/hooks/session-startup.sh
# Runs automatically via SessionStart hook when Claude Code launches.
# Handles: LLMGW auth check, MCP probe, Salesforce org check, org folder + audit status.
...
# --- 5. MCP Probe ---
if [ -f ".mcp.json" ]; then
  OUTPUT+="## ✅ MCP config present (.mcp.json)\n\n"
else
  OUTPUT+="## ⚠️ No .mcp.json found. MCP tools unavailable.\n"
  OUTPUT+="   Run /setup-lsdo-build or check that you're in the LSDO Build Template directory.\n\n"
fi

# --- 5b. First-Time Detection ---
if [ ! -d "orgs" ] || [ ! -f "orgs/sparring-lessons.md" ]; then
  OUTPUT+="## ℹ️ First time? Run /setup-lsdo-build to connect your org and initialize.\n\n"
fi

# --- 5c. CLAUDE.md Presence Check ---
if [ ! -f "CLAUDE.md" ]; then
  OUTPUT+="## ⚠️ No CLAUDE.md found. Are you in the LSDO Build Template project directory?\n\n"
fi
...
if git rev-parse --git-dir &>/dev/null; then
  if git fetch origin main --quiet --depth=1 2>/dev/null; then
    ...
    if [ -n "$LOCAL" ] && [ -n "$REMOTE" ] && [ "$LOCAL" != "$REMOTE" ]; then
      ...
      OUTPUT+="## ⚠️ LSDO Build Template update available ($COMMITS_BEHIND commit(s) behind) — /scout-sparring will prompt you.\n\n"
    else
      rm -f "$FLAG_FILE"
    fi
  fi
...
# --- 7. Ready ---
OUTPUT+="---\n"
OUTPUT+="**Ready.** Available commands:\n"
...
OUTPUT+="  /setup-lsdo-build — first-time project setup\n"
OUTPUT+="  /lsdo-readiness   — audit against LSDO best practices\n"
OUTPUT+="  /lsdo-deploy      — smart incremental deploy\n"
OUTPUT+="  /lsdo-data        — seed or extract demo data (SFDMU)\n"
OUTPUT+="  /lsdo-agent       — guided Agentforce agent creation\n"
OUTPUT+="  /lsdo-flow        — guided flow builder\n"
OUTPUT+="  /lsdo-cci         — generate CumulusCI YAML + package\n"
