...
5. **Standard actions first.** Before building custom actions, check the [Standard Actions Catalog](references/standard-actions-catalog.md) for a pre-built action that already does what you need. Salesforce ships 240+ standard actions across Sales, Service, Commerce, Field Service, Industries, and more. Standard actions are production-ready, maintained by Salesforce, and require no backing logic. Only build custom actions when no standard action covers the use case. When designing an Agent Spec, annotate each action as `STANDARD` (use existing), `NEEDS STUB` (build custom), or `EXISTS` (already in org).

6. **Standard prompt templates first.** Before building custom prompt templates, check the [Standard Prompt Templates Catalog](references/standard-prompt-templates-catalog.md) for a pre-built template. Salesforce ships 60+ standard templates across Sales, Service, Commerce, Field Service, and Industries. Standard templates can be cloned and customized in Prompt Builder. When an agent action needs AI-generated content (summaries, emails, field values), prefer invoking a standard prompt template via `generatePromptResponse://` over writing custom prompt logic.
...
11. [Standard Actions Catalog](references/standard-actions-catalog.md) — 240+ pre-built actions by category; check before building custom actions
12. [Standard Prompt Templates Catalog](references/standard-prompt-templates-catalog.md) — 60+ pre-built prompt templates by category; check before building custom templates
...
- [testing-agentforce](../testing-agentforce/SKILL.md) — write and run structured test suites for agents you build
- [observing-agentforce](../observing-agentforce/SKILL.md) — analyze production agent behavior with session traces
- [implementing-ui-bundle-agentforce-conversation-client](../implementing-ui-bundle-agentforce-conversation-client/SKILL.md) — embed agents into React UI bundles as chat widgets
- [generating-custom-lightning-type](../generating-custom-lightning-type/SKILL.md) — create CLTs for structured agent action inputs/outputs
- [generating-flow](../generating-flow/SKILL.md) — build Flows invoked as agent actions
